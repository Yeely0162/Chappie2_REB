...
void FunctionRoot() 
{
    if (server.hasArg("selectSSID")) {
        server.send(200, "text/html", ROOT_HTML + scanNetworksID + "</ul></div></div></div></div><script>function fillSSID(element) {var ssidValue = element.innerText || element.textContent; document.getElementById('ssid').value = ssidValue;}</script></body></html>");   //scanNetWprksID是扫描到的wifi
    } else {
        server.send(200, "text/html", ROOT_HTML + scanNetworksID + "</ul></div></div></div></div><script>function fillSSID(element) {var ssidValue = element.innerText || element.textContent; document.getElementById('ssid').value = ssidValue;}</script></body></html>");   
    }  
}
...
void scanWiFi() {
  ...
  if (n == 0) {
    ...
    for (int i = 0; i < n; ++i) {
      scanNetworksID += "<li onclick=\"fillSSID(this)\">" + WiFi.SSID(i) + "</li>";
      delay(10);
    }
  }
}
